...
{
    public class Collection : IServiceCollection
    {
        private bool _isReadOnly = false;
        private Type[] attrArr = { typeof(TransientAttribute), typeof(ScopeAttribute), typeof(SingletonAttribute) };
        public Dictionary<Type, List<Descriptor>> dictiontry = new Dictionary<Type, List<Descriptor>>();
        ...
        private Descriptor GetServiceDescriptorByIndex(int index)
        {
            int current = 0;
        ...
        private ServiceProvider InitinalServiceProvider()
        {
            ...
            descriptors.Add(Descriptor.Singleton(typeof(ServiceProvider), serviceProvider));
        ...
        private void RegistorTypes(List<TypeInfo> definedtypes)
        {
            ...
            foreach (TypeInfo implementationType in definedtypes)
            {
                ...
                Descriptor descriptor = new Descriptor(serviceType, implementationType, serviceLifetime);
                Add(descriptor);
            }
        }
