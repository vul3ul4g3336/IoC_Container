...
namespace IOCServiceCollection
{
    public class ServiceProvider : IServiceProvider
    {
        ...
        public object GetService(Type serviceType)
        {
            ...
            if (!_services.dictiontry.TryGetValue(serviceType, out List<Descriptor> descriptors))
            {
                // 檢查是否為IEnumerable
                if (serviceType.IsGenericType && serviceType.GetGenericTypeDefinition() == typeof(IEnumerable<>))
                {
                    ...
                    if (_services.dictiontry.TryGetValue(itemType, out List<Descriptor> genericDescriptors))
                    {
                        //取得List<>
                        ...
                        foreach (Descriptor genericDescriptor in genericDescriptors)
                        {
                ...
                else if (serviceType.IsGenericType) //ILogger<Form1>
                {
                    ...
                    //取得泛型的實作類別 Logger<T> 
                    if (_services.dictiontry.TryGetValue(genericType, out List<Descriptor> genericDescriptors))
                    {
                        //取得泛型內的類型Form1
                        Type gt = serviceType.GetGenericArguments()[0];

                        Descriptor genericDescriptor = genericDescriptors.Last();

                        //組裝泛型 Logger<Form1>
                        Type genericWithImplementationType = genericDescriptor.ImplementationType.MakeGenericType(gt);

                        //製作ServiceDescriptor
                        Descriptor serviceDescriptor = new Descriptor(genericDescriptor.ServiceType, genericWithImplementationType, genericDescriptor.Lifetime);
        ...
        public object GetImplementationInstance(Descriptor descriptor)
        {
